...
| B-03 | Board sidebar listing — collapsible, searchable | P0 | Sidebar persists across app restarts. If no boards exist (first launch), shows an empty-state onboarding view (see A-02, §8) instead of an empty list. Boards grouped into Areas (B-07, M-F) render under collapsible area headers, ungrouped boards first; the filter shows a flat, headerless list across ALL boards (collapsed areas included) |
...
| B-06 | Reorder boards in sidebar (drag-and-drop) | P1 | Native macOS row reorder (SwiftUI `List` `.onMove`); disabled while the sidebar filter is active; one undo step (⌘Z); order persists across relaunch. Added post-MVP. Since M-F, reorder is per-section (within an area, or within the ungrouped run); section-local offsets map onto the single global order via a pure helper (`Reordering.movedWithinSubset`), and reorder remains disabled while filtering |
| B-07 | Group boards into Areas — collapsible sidebar sections | P1 | Added post-MVP (M-F). Opt-in: boards are ungrouped by default; ungrouped boards render first, headerless. One level (no nesting), no completion semantics. Membership moves via the board row's context menu ("Move to Area ▸": areas, No Area, New Area…); area creation is find-or-create by exact trimmed name (case-sensitive — the import merge key), so duplicate names cannot exist; rename/collapse/delete via the area header's context menu. Collapse state is model state (the list-collapse precedent): see §4.7, persisted, exported (v5). Deleting an area RELEASES its boards to ungrouped (`.nullify` — boards are never deleted; confirmation-gated) and is **not undoable** (the fourth detach-and-clear delete — see §4.7, U-01). `Board.position` stays ONE global sequence (see §6.2): ⌘1–⌘9 keeps flat position order regardless of grouping/collapse, and selecting a board inside a collapsed area auto-expands it. Within-section reorder drag is manually verified (the B-06 limitation, §9.8); cross-section drag is not supported — the context menu is the cross-area path |
...
All mutating operations — create/rename/move/reorder of boards, lists, and cards; label toggles; due-date edits — are undoable, backed by the standard `UndoManager` and exposed through the standard Edit menu and ⌘Z/⇧⌘Z. The exceptions are **all four delete operations — board, list, card, and area deletion**, none of which is undoable: board deletion because an on-disk Board delete's undo snapshotting fatally asserts (see B-02), list/card deletion since **M-E** because the same class of fatal assert reproduces one cascade level shallower once a card carries checklist items (the `ChecklistUndoOnDiskTests` spike, Task 0 of the M-E plan; see §4.3 C-05, §4.2 L-02), and area deletion since **M-F** for the identical reason one relationship-collection lighter still — the `.nullify` release of an area's boards silently loses restored membership under SwiftData's automatic undo registration (the `AreaUndoOnDiskTests` spike; see B-07). Any delete — board, list, card, or area — **clears the undo stack outright**, since earlier registered groups may reference the now-deleted rows; an area delete's boards are never themselves deleted, only released to ungrouped. Board/card *creation* undo is unaffected and works normally. Board-to-area moves (M-F `setArea`) remain fully undoable despite the delete finding above — they route through an explicitly registered inverse pair (the `moveCard` cross-list mechanism, `setArea`'s second use of the pattern) instead of SwiftData's automatic registration, which is exactly the machinery the spike found unsafe for this shape.

| # | Feature | Priority | Notes |
|---|---------|----------|-------|
| U-01 | Undo/redo for all mutations | P0 | `UndoManager`-backed (NSUndoManager); standard Edit menu items; ⌘Z / ⇧⌘Z. Session undo history holds at least 50 steps (see §10, N-04). **All four deletes — board (B-02), list (L-02), card (C-05), and area (B-07) — are excluded and not undoable**: board deletion because SwiftData's undo snapshotting of an on-disk Board delete fatally asserts (crash-forced platform limitation; see B-02 and §7 roadmap), list/card deletion since M-E because the identical fatal-assert class reproduces one cascade level shallower once checklist items exist (`ChecklistUndoOnDiskTests`, M-E Task 0), and area deletion since M-F for the same reason one relationship-collection lighter (`AreaUndoOnDiskTests`) — boards are never deleted by it, only released to ungrouped. Each delete clears the undo stack outright. Board-to-area moves (`setArea`) ARE undoable via an explicitly registered inverse pair rather than automatic registration (the `moveCard` cross-list precedent). All other board/list/card/area mutations, including creation, remain undoable |
...
- `area`: Area? (optional; nil = ungrouped — the default)
...
| Area | Releases its Boards to ungrouped (nullify); Boards are untouched |

All of the above are undoable (see §4.7, U-01), **except all four deletes — Board, List, Card, and Area** — none of which is undoable: Board deletion is a SwiftData platform limitation (undo snapshotting of an on-disk Board delete fatally asserts), List/Card deletion joined it since **M-E**, once cards carry checklist items (the same fatal-assert class reproduces one cascade level shallower — see the `ChecklistUndoOnDiskTests` spike), and Area deletion joined it since **M-F** for the same reason one relationship-collection lighter (the `AreaUndoOnDiskTests` spike — undo correctly restores the deleted Area row but silently drops its restored boards' membership). Board and List deletes both require a confirmation dialog before the delete is committed (see B-02, L-02); neither is undoable. Card delete has no confirmation dialog and, since M-E, is not undoable either — deletion is now permanent and the undo stack clears (see C-05). Area delete also requires a confirmation dialog (see B-07) and is not undoable; unlike the other three, its blast radius is strictly smaller — Boards are never deleted, only released to ungrouped.
...
| Soft-delete/restore for boards, lists, cards, and areas (undo replacement) | P1 | Platform-forced deferral: board, list, and area delete are confirmation-gated and card delete has no confirmation dialog at all, but none of the four is undoable — SwiftData's undo snapshotting of an on-disk cascade delete (or, for Area's nullify release, the restored membership after it) fatally asserts or silently drops rows under undo (see B-02, L-02, C-05, B-07, U-01). A soft-delete + restore ("Recently Deleted") flow would give back reversibility across all four without relying on `UndoManager` |
...
- **E-02 — Import from JSON.** (a) Given a previously exported JSON file, when the user imports it, then the boards/lists/cards/labels/due dates/themes, and area grouping it contains are restored in the same order as the source. (b) Add to Existing is **not undoable** — per the recorded spike outcome (§4.6), it detaches the undo manager and clears the undo stack, exactly like board delete; ⌘Z after an Add import does nothing. (c) Replace All Boards deletes the existing boards first, is also not undoable and clears the undo stack, and is unavailable for a zero-board backup. (d) Given an invalid file (malformed JSON, missing required fields, or an unsupported `formatVersion`), when the user attempts to import it, then nothing is imported and an error is shown stating the existing boards are unchanged. (e) Given a v5 export containing areas, importing it Add-mode into a store with a same-named area merges membership into the existing area (exact-name match) and keeps that area's local collapse state; Replace All leaves exactly the backup's areas and no others. (The `NSOpenPanel` leg is manually verified, not XCUITest-driven; content correctness is automated via `--import-from` — see §9.8.)

### 9.7 Undo/Redo

- **U-01 — Undo/redo for all mutations.** Given any undoable mutating action (create/rename/move/reorder of a board, list, card, or area; label toggle; due-date edit), when the user presses ⌘Z, then that single action is fully reverted, including position/order; when the user then presses ⇧⌘Z, then the action is reapplied. This holds for at least 50 consecutive undoable actions in one session (see §10, N-04). **Board, list, card, and area deletion are all excluded** — none is undoable (board: a SwiftData platform limitation; list/card: the same fatal-assert class since M-E, once cards carry checklist items; area: the same class since M-F, one relationship-collection lighter — see §4.7), and each clears the undo stack outright; undoing a board/list/card/area *creation* is supported and, after it, the app remains responsive. Board-to-area moves (`setArea`) remain undoable despite the delete exclusion above — they route through an explicitly registered inverse pair rather than SwiftData's automatic registration (see §4.7).
...
- B-07 Group boards into Areas (collapsible sidebar sections — see §4.1; area delete confirm-only, not undoable — see §4.7)
