...
| Soft-delete/restore for boards, lists, and cards (undo replacement) | P1 | Platform-forced deferral: board and list delete are confirmation-gated and card delete has no confirmation dialog at all, but none of the three is undoable — SwiftData's undo snapshotting of an on-disk cascade delete fatally asserts (see B-02, L-02, C-05, U-01). A soft-delete + restore ("Recently Deleted") flow would give back reversibility across all three without relying on `UndoManager` |
