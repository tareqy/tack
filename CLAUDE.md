...
- **Native `List` `.onMove` row-reorder is not XCUITest-automatable** (confirmed for the B-06 sidebar): the synthesized `press(forDuration:thenDragTo:)` gesture visibly reorders the row mid-drag but the drop never commits (NSTableView's drag session doesn't call `performDragOperation`), unlike a real/CGEvent-driven drag, which does commit — verified by hand. `SidebarReorderUITests` keeps only the filter-gate test; the drag itself is unit-tested (`Reordering`, `BoardStore.moveBoards`) and manually verified, not e2e-covered.
