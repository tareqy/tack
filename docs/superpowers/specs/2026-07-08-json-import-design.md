...
- **Menu:** File ▸ **Import Boards…** (⇧⌘I, symmetric with ⇧⌘E), directly after Export All Boards… in the existing scene-level `CommandGroup`. Routed through `guardedMutation`; enabled whenever `boardSelection` is published — effectively always, **including at zero boards** (restore-into-empty is the headline case). New `BoardSelectionActions.importBoards: () -> Void` closure — a `Commands` value cannot present a `.fileImporter`, so the command flips `RootView` state (the E-01 pattern).
