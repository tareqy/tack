...
**Outcome: the risk materialized.** `SidebarReorderUITests` was implemented with all three tests from this spec; the two drag tests (`testDragReorderPersistsAcrossRelaunch`, `testUndoRestoresOrderAfterDrag`) failed consistently. A `.xcresult` screen-recording review showed the row visually reordering mid-drag (SwiftUI's live drag-over-reorder preview) but the order snapping back to the seeded Groceries/Work on release — the drop never commits. Retrying with `pressDuration`/`holdDuration` tuned to 0.9s/1.0s (per the base class helper's parameters) made no difference. Manual verification confirmed the feature itself works: a CGEvent-driven synthetic drag (mouseDown → 40 interpolated `leftMouseDragged` steps over the row → mouseUp), which is indistinguishable from real hardware input at the HID level and bypasses XCUITest's own gesture-synthesis pipeline entirely, DID commit the reorder against a manually-launched build (`.build/DerivedData/Build/Products/Debug/Tack.app`, `--uitest --fixture standard`). This isolates the failure to XCUITest's `press(forDuration:thenDragTo:withVelocity:thenHoldForDuration:)` gesture specifically — most likely its synthetic events don't pump through AppKit's drag-session tracking loop the way real hardware (or raw CGEvent-posted) events do, so `NSTableView` never calls the final `performDragOperation`. Per the fallback: `testDragReorderPersistsAcrossRelaunch` and `testUndoRestoresOrderAfterDrag` were deleted from `TackUITests/SidebarReorderUITests.swift`; `testFilterDisablesReorder` (which only asserts absence of movement, so it isn't exposed to this limitation) was kept and passes. B-06's drag-reorder is manually verified working; unit coverage (`Reordering`, `BoardStore.moveBoards`, including its one-undo-step behavior) remains the automated regression backstop.
